...
[[ -d "$SRC" ]] || { echo "C++ source not found at $SRC" >&2; exit 1; }

MD=0
[[ "${1:-}" == "--md" ]] && MD=1

row() { # name cpp go note
  if (( MD )); then printf '| %s | %s | %s | %s |\n' "$1" "$2" "$3" "$4"
...
row "source files" "$cpp_files" "$go_files" "$(( go_files * 100 / cpp_files ))%"
row "lines of code" "$cpp_loc" "$go_loc" "$(( go_loc * 100 / cpp_loc ))%"
...
go_out=$( { grep -rhoE 'AddByte\(0x[0-9A-Fa-f]{2}\)' "$GO_ROOT/internal/protocol/" |
              grep -oE '0x[0-9A-Fa-f]{2}';
            grep -rhoE '\bop[A-Z][A-Za-z]*[[:space:]]*=[[:space:]]*0x[0-9A-Fa-f]{2}' "$GO_ROOT/internal/protocol/" |
              grep -oE '0x[0-9A-Fa-f]{2}'; } | tr 'a-f' 'A-F' | sort -u | wc -l | tr -d ' ')
...
row "enum globals" "1310" "1490" "0 missing, 0 wrong (snapshot)"
...
done < <(grep -ohE 'CREATE TABLE IF NOT EXISTS `[a-z_]+`' "$GO_ROOT/schema/schema.sql" |
         sed 's/.*`\(.*\)`/\1/' | sort -u)
total=$(grep -cE 'CREATE TABLE IF NOT EXISTS' "$GO_ROOT/schema/schema.sql")
...
cmp_pair() { # label cpp-path go-path
  local c g
  c=$(cat "$SRC/$2" 2>/dev/null | wc -l | tr -d ' ')
  g=$(cat "$GO_ROOT/$3" 2>/dev/null | wc -l | tr -d ' ')
  [[ "$c" == "0" ]] && return
  row "$1" "$c" "$g" "$(( g * 100 / c ))%"
}
cmp_pair "monster AI"      "creatures/monsters/monster.cpp"        "internal/game/ai_engine.go"
